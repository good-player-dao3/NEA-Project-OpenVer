...
const originRoot = resolve(repositoryRoot, "origin/origin/origin");
const lokiboxRoot = resolve(repositoryRoot, "Lokibox/lokibox-merged-bypass/src");
...
const lokiboxFiles = await sourceFiles(lokiboxRoot, /\.(?:ts|svelte)$/u);
const originText = await joinedText(originFiles);
const lokiboxText = await joinedText(lokiboxFiles);
...
const lokiboxShapeWrites = fieldWrites(lokiboxText);
...
const coverage = {
  ...
  indexedSourceSets: [
    ...
    sourceSet("lokibox-runtime-adapters", lokiboxRoot, lokiboxFiles),
    {
