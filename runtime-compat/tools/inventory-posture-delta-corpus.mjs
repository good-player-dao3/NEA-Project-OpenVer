...
const captureRoot = resolve(siblingRoot, "box-go", "dump", "downloaded");
const packetAnalyzerPath = resolve(siblingRoot, "box-go", "src", "packet-analysis.ts");
...
const inventory = {
  ...
  sourceSets: [
    { id: "box-go-captures", path: "sibling-box-go/dump/downloaded", files: captures.length, available: captures.length > 0 },
    { id: "box-go-resource-archives", path: "sibling-box-go/dump/downloaded", files: archives.length, available: archives.length > 0 },
  ...
  packetOrderEvidence: {
    path: "sibling-box-go/src/packet-analysis.ts",
...
async function inventoryCaptures() {
  let names;
  try {
    names = (await readdir(captureRoot)).filter(name => /^box-go-(?:browser-)?\d.*\.json$/u.test(name)).sort();
  } catch (error) {
...
async function inventoryStaticReplay() {
  const path = resolve(legacyRoot, "others", "box-go-src", "replay-data.ts");
  try {
    const bytes = await readFile(path);
    ...
    return {
      path: "sibling-local-worktree/others/box-go-src/replay-data.ts",
      available: true,
      bytes: bytes.length,
      sha256: sha256(bytes),
    ...
    if (error?.code === "ENOENT") return { path: "sibling-local-worktree/others/box-go-src/replay-data.ts", available: false, decodedMessages: 0, byMessage: {}, rawPayloadAvailable: false };
    throw error;
  }
}
