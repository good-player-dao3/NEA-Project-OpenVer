...
- The executable fixture under `runtime-compat/conformance/` describes only these directly observed behaviors; it is not a substitute implementation for unrecovered APIs.
...
The executable registry is generated at `runtime-compat/abi/runtime-contracts.json`. It binds the five layers, both script contracts, MuDB protocol dependencies, authoritative state flows, Demo script usage and resolved ABI entry identifiers. `runtime-compat/abi/current-runtime.json` is also generated from the Player and local Server Runtime analyzers rather than maintained as a hand-written subset.
