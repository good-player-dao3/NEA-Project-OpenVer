...
Literal server entity selectors are checked against the recovered `ParsedSelector` grammar. A tag or id token containing whitespace is reported as `partial` because whitespace is not CSS-style intersection syntax, and an unprefixed component other than `player` / `entity` is `partial` because the historical `testComponent` helper has not been recovered. Dynamic selector expressions remain executable and are not blocked solely for being dynamic.

Server storage variables returned by `storage.getDataStorage()` are propagated as `GameDataStorage`, and variables returned by `.list()` are propagated as `QueryList`, so their member requirements appear in the Capability Manifest instead of being treated as map-defined properties. `storage.getGroupStorage()` is a blocking diagnostic unless the embedding supplies `groupStorageEnabled: true` together with an authoritative group-scoped provider.
