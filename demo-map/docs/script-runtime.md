...
Player positions represent the rigid-body center. Each server player receives an explicit collision profile containing origin status, size status, broadphase bounds and shape half extents; no solver default silently supplies a body size. The Demo uses the recovered upright Player default `0.45 / 1.1 / 0.45` for both sets until a historical posture delta proves otherwise. Network PUBLIC bodies populate `rx/ry/rz` from `boundsHalfExtents` and `hsx/hsy/hsz` from `shapeHalfExtents`. Candidate lookup uses bounds while actual contacts and triggers use shape. Player snapshots expose both sets plus `collision.aabb` and `collision.shapeAabb`. Mutations such as `player.applyImpulse()` and `player.damage()` require `server.player.write`.
